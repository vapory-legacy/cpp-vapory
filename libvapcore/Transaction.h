...
public:
	...
	/// @returns the amount of VAP to be transferred by this (message-call) transaction, in Wei. Synonym for endowment().
	u256 value() const { return m_value; }

	/// @returns the base fee and thus the implied exchange rate of VAP to GAS.
	...
	int64_t baseGasRequired(VVMSchedule const& _es) const { return baseGasRequired(isCreation(), &m_data, _es); }

	/// Get the fee associated for a transaction with the given data.
	static int64_t baseGasRequired(bool _contractCreation, bytesConstRef _data, VVMSchedule const& _es);
...
protected:
	...
	u256 m_value;						///< The amount of VAP to be transferred by this transaction. Called 'endowment' for contract-creation transactions.
	Address m_receiveAddress;			///< The receiving address of the transaction.
	u256 m_gasPrice;					///< The base fee and thus the implied exchange rate of VAP to GAS.
	...
	int m_chainId = -4;					///< EIP155 value for calculating transaction hash https://github.com/vaporyco/EIPs/issues/155
