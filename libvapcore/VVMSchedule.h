...
struct VVMSchedule
{
	VVMSchedule(): tierStepGas(std::array<unsigned, 8>{{0, 2, 3, 5, 8, 10, 20, 0}}) {}
	VVMSchedule(bool _efcd, bool _hdc, unsigned const& _txCreateGas): exceptionalFailedCodeDeposit(_efcd), haveDelegateCall(_hdc), tierStepGas(std::array<unsigned, 8>{{0, 2, 3, 5, 8, 10, 20, 0}}), txCreateGas(_txCreateGas) {}
...
static const VVMSchedule DefaultSchedule = VVMSchedule();
static const VVMSchedule FrontierSchedule = VVMSchedule(false, false, 21000);
static const VVMSchedule HomesteadSchedule = VVMSchedule(true, true, 53000);

static const VVMSchedule EIP150Schedule = []
{
	VVMSchedule schedule = HomesteadSchedule;
...
static const VVMSchedule EIP158Schedule = []
{
	VVMSchedule schedule = EIP150Schedule;
...
static const VVMSchedule ByzantiumSchedule = []
{
	VVMSchedule schedule = EIP158Schedule;
	schedule.haveRevert = true;
	schedule.haveReturnData = true;
	schedule.haveStaticCall = true;
	schedule.blockRewardOverwrite = {3 * vapor};
	return schedule;
}();

static const VVMSchedule EWASMSchedule = []
{
    VVMSchedule schedule = ByzantiumSchedule;
...
static const VVMSchedule ConstantinopleSchedule = []
{
	VVMSchedule schedule = ByzantiumSchedule;
