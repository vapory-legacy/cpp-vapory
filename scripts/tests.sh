...
#------------------------------------------------------------------------------
# Bash script to execute the cpp-vapory tests.
#
...
if [[ "$TESTS" == "On" ]]; then
    ...
    $BUILD_ROOT/test/testvap -- --testpath $BUILD_ROOT/../test/jsontests

    # Fill some state tests and make sure the result passes linting
    echo "Running testvap filler tests..."
    $BUILD_ROOT/test/testvap -t GeneralStateTests/stExample -- --filltests --testpath $BUILD_ROOT/../test/jsontests
    $BUILD_ROOT/test/testvap -t 'TransitionTests/bcEIP158ToByzantium' -- --filltests --singletest  ByzantiumTransition --testpath $BUILD_ROOT/../test/jsontests
    ...
    # enough time to build LLVM, build VVMJIT and run the tests twice within
    # the 48 minute absolute maximum run time for TravisCI.

    # Disabled until VVMJIT will catch up with Metropolis features.
    # if [[ "$OSTYPE" != "darwin"* ]]; then
    #     $BUILD_ROOT/test/testvap -t "VMTests*,StateTests*" -- --vm jit --testpath $BUILD_ROOT/../test/jsontests
    # fi
