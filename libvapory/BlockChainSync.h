...
{
public:
	BlockChainSync(VaporyHost& _host);
	~BlockChainSync();
	...
	void onPeerStatus(std::shared_ptr<VaporyPeer> _peer);

	/// Called by peer once it has new block headers during sync
	void onPeerBlockHeaders(std::shared_ptr<VaporyPeer> _peer, RLP const& _r);

	/// Called by peer once it has new block bodies
	void onPeerBlockBodies(std::shared_ptr<VaporyPeer> _peer, RLP const& _r);

	/// Called by peer once it has new block bodies
	void onPeerNewBlock(std::shared_ptr<VaporyPeer> _peer, RLP const& _r);

	void onPeerNewHashes(std::shared_ptr<VaporyPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes);
...
private:
	...
	VaporyHost& host() { return m_host; }
	VaporyHost const& host() const { return m_host; }

	void resetSync();
	void syncPeer(std::shared_ptr<VaporyPeer> _peer, bool _force);
	void requestBlocks(std::shared_ptr<VaporyPeer> _peer);
	void clearPeerDownload(std::shared_ptr<VaporyPeer> _peer);
	void clearPeerDownload();
	void collectBlocks();
	bool requestDaoForkBlockHeader(std::shared_ptr<VaporyPeer> _peer);
...
private:
	...
	std::set<std::weak_ptr<VaporyPeer>, std::owner_less<std::weak_ptr<VaporyPeer>>> m_daoChallengedPeers; ///> Peers to which we've sent DAO challenge request
	...
	std::map<std::weak_ptr<VaporyPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<VaporyPeer>>> m_headerSyncPeers; ///< Peers to m_downloadingSubchain number map
	std::map<std::weak_ptr<VaporyPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<VaporyPeer>>> m_bodySyncPeers; ///< Peers to m_downloadingSubchain number map
