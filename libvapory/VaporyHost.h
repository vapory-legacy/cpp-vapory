...
#include <libvapory/BlockChainSync.h>
#include "CommonNet.h"
...
struct VaporyHostTrace: public LogChannel { static const char* name(); static const int verbosity = 6; };

/**
 * @brief The VaporyHost class
 * @warning None of this is thread-safe. You have been warned.
 * @doWork Syncs to peers and sends new blocks and transactions.
 */
class VaporyHost: public p2p::HostCapability<VaporyPeer>, Worker
{
public:
	/// Start server, but don't listen.
	VaporyHost(BlockChain const& _ch, OverlayDB const& _db, TransactionQueue& _tq, BlockQueue& _bq, u256 _networkId);
	...
	void foreachPeer(std::function<bool(std::shared_ptr<VaporyPeer>)> const& _f) const;
...
private:
	...
	std::tuple<std::vector<std::shared_ptr<VaporyPeer>>, std::vector<std::shared_ptr<VaporyPeer>>, std::vector<std::shared_ptr<p2p::SessionFace>>> randomSelection(unsigned _percent = 25, std::function<bool(VaporyPeer*)> const& _allow = [](VaporyPeer const*){ return true; });
	...
	OverlayDB const& m_db;					///< References to DB, needed for some of the Vapory Protocol responses.
	...
	std::shared_ptr<VaporyPeerObserverFace> m_peerObserver;
};
