...
#define VAP_TIMED_ENACTMENTS 0

static const unsigned c_maxSyncTransactions = 1024;

const char* BlockSafeExceptions::name() { return VapViolet "⚙" VapBlue " ℹ"; }
const char* BlockDetail::name() { return VapViolet "⚙" VapWhite " ◌"; }
const char* BlockTrace::name() { return VapViolet "⚙" VapGray " ◎"; }
const char* BlockChat::name() { return VapViolet "⚙" VapWhite " ◌"; }

namespace
{

class DummyLastBlockHashes: public vap::LastBlockHashesFace
{
public:
...
bool Block::sync(BlockChain const& _bc, h256 const& _block, BlockHeader const& _bi)
{
	...
	else
	{
		...
		if (m_state.db().lookup(bi.stateRoot()).empty())	// TODO: API in State for this?
		{
			...
			cwarn << "Try rescuing the database by running: vap --rescue";
...
//		ex << errinfo_vmtrace(vmTrace(_block.block, _bc, ImportRequirements::None));
	...
	bool removeEmptyAccounts = m_currentBlock.number() >= _bc.chainParams().EIP158ForkBlock; // TODO: use VVMSchedule
