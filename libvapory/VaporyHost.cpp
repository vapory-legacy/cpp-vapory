...
{
public:
	VaporyPeerObserver(shared_ptr<BlockChainSync> _sync, TransactionQueue& _tq): m_sync(_sync), m_tq(_tq) {}
	...
	m_hostData(make_shared<VaporyHostData>(m_chain, m_db))
{
	...
	m_peerObserver = make_shared<VaporyPeerObserver>(m_sync, m_tq);
...
shared_ptr<Capability> VaporyHost::newPeerCapability(shared_ptr<SessionFace> const& _s, unsigned _idOffset, p2p::CapDesc const& _cap)
{
	auto ret = HostCapability<VaporyPeer>::newPeerCapability(_s, _idOffset, _cap);
