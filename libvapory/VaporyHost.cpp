...
unsigned const VaporyHost::c_oldProtocolVersion = 62; //TODO: remove this once v63+ is common
static unsigned const c_maxSendTransactions = 256;

char const* const VaporyHost::s_stateNames[static_cast<int>(SyncState::Size)] = {"NotSynced", "Idle", "Waiting", "Blocks", "State"};

#if defined(_WIN32)
const char* VaporyHostTrace::name() { return VapPurple "^" VapGray "  "; }
#else
const char* VaporyHostTrace::name() { return VapPurple "⧫" VapGray " "; }
#endif
...
namespace
{
class VaporyPeerObserver: public VaporyPeerObserverFace
{
public:
	VaporyPeerObserver(BlockChainSync& _sync, RecursiveMutex& _syncMutex, TransactionQueue& _tq): m_sync(_sync), m_syncMutex(_syncMutex), m_tq(_tq) {}

	void onPeerStatus(std::shared_ptr<VaporyPeer> _peer) override
	{
		...
		{
			// "fix" for https://github.com/vaporyco/webthree-umbrella/issues/300
			clog(NetWarn) << "Failed invariant during sync, restarting sync";
			m_sync.restartSync();
		}
	}

	void onPeerTransactions(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) override
	{
		unsigned itemCount = _r.itemCount();
		clog(VaporyHostTrace) << "Transactions (" << dec << itemCount << "entries)";
	...
	void onPeerBlockHeaders(std::shared_ptr<VaporyPeer> _peer, RLP const& _headers) override
	{
		RecursiveGuard l(m_syncMutex);
		try
		{
	...
	void onPeerBlockBodies(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) override
	{
		RecursiveGuard l(m_syncMutex);
		try
		{
	...
	void onPeerNewHashes(std::shared_ptr<VaporyPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes) override
	{
		RecursiveGuard l(m_syncMutex);
		try
		{
	...
	void onPeerNewBlock(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) override
	{
		RecursiveGuard l(m_syncMutex);
		try
		{
	...
	void onPeerNodeData(std::shared_ptr<VaporyPeer> /* _peer */, RLP const& _r) override
	{
		unsigned itemCount = _r.itemCount();
		clog(VaporyHostTrace) << "Node Data (" << dec << itemCount << "entries)";
	}

	void onPeerReceipts(std::shared_ptr<VaporyPeer> /* _peer */, RLP const& _r) override
	{
		unsigned itemCount = _r.itemCount();
		clog(VaporyHostTrace) << "Receipts (" << dec << itemCount << "entries)";
	}
...
class VaporyHostData: public VaporyHostDataFace
{
public:
	VaporyHostData(BlockChain const& _chain, OverlayDB const& _db): m_chain(_chain), m_db(_db) {}
...
VaporyHost::VaporyHost(BlockChain const& _ch, OverlayDB const& _db, TransactionQueue& _tq, BlockQueue& _bq, u256 _networkId):
	HostCapability<VaporyPeer>(),
	Worker		("vapsync"),
	m_chain		(_ch),
	m_db(_db),
	m_tq		(_tq),
	...
	m_hostData(make_shared<VaporyHostData>(m_chain, m_db))
{
	// TODO: Composition would be better. Left like that to avoid initialization
	//       issues as BlockChainSync accesses other VaporyHost members.
	m_sync.reset(new BlockChainSync(*this));
	m_peerObserver = make_shared<VaporyPeerObserver>(*m_sync, x_sync, m_tq);
	m_latestBlockSent = _ch.currentHash();
	m_tq.onImport([this](ImportResult _ir, h256 const& _h, h512 const& _nodeId) { onTransactionImported(_ir, _h, _nodeId); });
	allowVptrAccess();
}

VaporyHost::~VaporyHost()
{
	terminate();
}

bool VaporyHost::ensureInitialised()
{
	if (!m_latestBlockSent)
	{
		// First time - just initialise.
		m_latestBlockSent = m_chain.currentHash();
		clog(VaporyHostTrace) << "Initialising: latest=" << m_latestBlockSent;
...
void VaporyHost::completeSync()
{
	RecursiveGuard l(x_sync);
	m_sync->completeSync();
}

void VaporyHost::doWork()
{
	...
	{
		m_lastTick = now;
		foreachPeer([](std::shared_ptr<VaporyPeer> _p) { _p->tick(); return true; });
	}
...
void VaporyHost::maintainTransactions()
{
	// Send any new transactions.
	unordered_map<std::shared_ptr<VaporyPeer>, std::vector<size_t>> peerTransactions;
	auto ts = m_tq.topTransactions(c_maxSendTransactions);
	{
		...
		for (size_t i = 0; i < ts.size(); ++i)
		{
			...
			auto peers = get<1>(randomSelection(0, [&](VaporyPeer* p) { return p->m_requireTransactions || (unsent && !p->m_knownTransactions.count(t.sha3())); }));
	...
	foreachPeer([&](shared_ptr<VaporyPeer> _p)
	{
		bytes b;
		unsigned n = 0;
		...
		if (n || _p->m_requireTransactions)
		{
			...
			_p->sealAndSend(ts);
			clog(VaporyHostTrace) << "Sent" << n << "transactions to " << _p->session()->info().clientVersion;
		}
...
void VaporyHost::foreachPeer(std::function<bool(std::shared_ptr<VaporyPeer>)> const& _f) const
{
	...
	for (auto s: sessions)
		if (!_f(capabilityFromSession<VaporyPeer>(*s.first)))
			return;

	sessions = peerSessions(c_oldProtocolVersion); //TODO: remove once v61+ is common
	std::sort(sessions.begin(), sessions.end(), sessionLess);
	for (auto s: sessions)
		if (!_f(capabilityFromSession<VaporyPeer>(*s.first, c_oldProtocolVersion)))
			return;
}

tuple<vector<shared_ptr<VaporyPeer>>, vector<shared_ptr<VaporyPeer>>, vector<shared_ptr<SessionFace>>> VaporyHost::randomSelection(unsigned _percent, std::function<bool(VaporyPeer*)> const& _allow)
{
	vector<shared_ptr<VaporyPeer>> chosen;
	vector<shared_ptr<VaporyPeer>> allowed;
	vector<shared_ptr<SessionFace>> sessions;

	size_t peerCount = 0;
	foreachPeer([&](std::shared_ptr<VaporyPeer> _p)
	{
		if (_allow(_p.get()))
		{
...
void VaporyHost::maintainBlocks(h256 const& _currentHash)
{
	// Send any new blocks.
	...
	if (detailsFrom.totalDifficulty < detailsTo.totalDifficulty)
	{
		if (diff(detailsFrom.number, detailsTo.number) < 20)
		{
			...
			clog(VaporyHostTrace) << "Sending a new block (current is" << _currentHash << ", was" << m_latestBlockSent << ")";

			h256s blocks = get<0>(m_chain.treeRoute(m_latestBlockSent, _currentHash, false, false, true));

			auto s = randomSelection(25, [&](VaporyPeer* p){
				DEV_GUARDED(p->x_knownBlocks)
					return !p->m_knownBlocks.count(_currentHash);
				return false;
			});
			for (shared_ptr<VaporyPeer> const& p: get<0>(s))
			...
			for (shared_ptr<VaporyPeer> const& p: get<1>(s))
			{
				RLPStream ts;
...
bool VaporyHost::isSyncing() const
{
	return m_sync->isSyncing();
}

SyncStatus VaporyHost::status() const
{
	RecursiveGuard l(x_sync);
	return m_sync->status();
}

void VaporyHost::onTransactionImported(ImportResult _ir, h256 const& _h, h512 const& _nodeId)
{
	auto session = host()->peerSession(_nodeId);
	if (!session)
		return;

	std::shared_ptr<VaporyPeer> peer = capabilityFromSession<VaporyPeer>(*session);
	if (!peer)
		peer = capabilityFromSession<VaporyPeer>(*session, c_oldProtocolVersion);
	if (!peer)
		return;
...
shared_ptr<Capability> VaporyHost::newPeerCapability(shared_ptr<SessionFace> const& _s, unsigned _idOffset, p2p::CapDesc const& _cap, uint16_t _capID)
{
	auto ret = HostCapability<VaporyPeer>::newPeerCapability(_s, _idOffset, _cap, _capID);

	auto cap = capabilityFromSession<VaporyPeer>(*_s, _cap.second);
	assert(cap);
	cap->init(
		protocolVersion(),
		m_networkId,
