...
#include <libvapory/CodeSizeCache.h>
...
/**
 * Model of an Vapory state, essentially a facade for the trie.
 *
...
public:
	...
	/// Populate the state from the given AccountMap. Just uses dev::vap::commit().
	void populateFrom(AccountMap const& _map);

	/// @returns the set containing all addresses currently in use in Vapory.
	/// @warning This is slowslowslow. Don't use it unless you want to lock the object for seconds or minutes at a time.
	/// @throws InterfaceNotSupported if compiled without VAP_FATDB.
