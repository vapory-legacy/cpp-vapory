...
#if defined(_WIN32)
const char* BlockChainDebug::name() { return VapBlue "8" VapWhite " <>"; }
const char* BlockChainWarn::name() { return VapBlue "8" VapOnRed VapBlackBold " X"; }
const char* BlockChainNote::name() { return VapBlue "8" VapBlue " i"; }
const char* BlockChainChat::name() { return VapBlue "8" VapWhite " o"; }
#else
const char* BlockChainDebug::name() { return VapBlue "☍" VapWhite " ◇"; }
const char* BlockChainWarn::name() { return VapBlue "☍" VapOnRed VapBlackBold " ✘"; }
const char* BlockChainNote::name() { return VapBlue "☍" VapBlue " ℹ"; }
const char* BlockChainChat::name() { return VapBlue "☍" VapWhite " ◌"; }
#endif

std::ostream& dev::vap::operator<<(std::ostream& _out, BlockChain const& _bc)
{
...
ldb::Slice dev::vap::toSlice(h256 const& _h, unsigned _sub)
{
...
ldb::Slice dev::vap::toSlice(uint64_t _n, unsigned _sub)
{
#if ALL_COMPILERS_ARE_CPP11_COMPLIANT
	...
	if (!m_blocksDB || !m_extrasDB)
	{
		...
		else
		{
			cwarn <<
				...
				"already open. You appear to have another instance of vapory running. Bailing.";
...
//	_bq.tick(*this);
	...
	for (VerifiedBlock const& block: blocks)
	{
		do {
			...
			catch (dev::vap::AlreadyHaveBlock const&)
			{
				cwarn << "ODD: Import queue contains already imported block";
				continue;
			}
			catch (dev::vap::UnknownParent)
			{
...
Block BlockChain::genesisBlock(OverlayDB const& _db) const
{
	...
	{
		ret.noteChain(*this);
		dev::vap::commit(m_params.genesisState, ret.mutableState().m_state);		// bit horrible. maybe consider a better way of constructing it?
