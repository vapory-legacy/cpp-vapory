...
/**
 * @brief Main API hub for interfacing with Vapory.
 */
...
public:
	...
	/// Get the block.
	dev::vap::Block block(h256 const& _blockHash, PopulationStatistics* o_stats) const;

	/// Get the object representing the current state of Vapory.
	dev::vap::Block postState() const { ReadGuard l(x_postSeal); return m_postSeal; }
	...
	std::unique_ptr<StateImporterFace> createStateImporter() { return dev::vap::createStateImporter(m_stateDB); }
	std::unique_ptr<BlockChainImporterFace> createBlockChainImporter() { return dev::vap::createBlockChainImporter(m_bc); }
...
protected:
	...
	std::weak_ptr<VaporyHost> m_host;		///< Our Vapory Host. Don't do anything if we can't lock.
