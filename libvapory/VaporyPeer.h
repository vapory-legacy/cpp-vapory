...
class VaporyPeerObserverFace
{
public:
	virtual ~VaporyPeerObserverFace() {}

	virtual void onPeerStatus(std::shared_ptr<VaporyPeer> _peer) = 0;

	virtual void onPeerTransactions(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerBlockHeaders(std::shared_ptr<VaporyPeer> _peer, RLP const& _headers) = 0;

	virtual void onPeerBlockBodies(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerNewHashes(std::shared_ptr<VaporyPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes) = 0;

	virtual void onPeerNewBlock(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerNodeData(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerReceipts(std::shared_ptr<VaporyPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerAborting() = 0;
};

class VaporyHostDataFace
{
public:
	virtual ~VaporyHostDataFace() {}
...
/**
 * @brief The VaporyPeer class
 * @todo Document fully.
 * @todo make state transitions thread-safe.
 */
class VaporyPeer: public p2p::Capability
{
	friend class VaporyHost; //TODO: remove this
	friend class BlockChainSync; //TODO: remove this

public:
	/// Basic constructor.
	VaporyPeer(std::shared_ptr<p2p::SessionFace> _s, p2p::HostCapabilityFace* _h, unsigned _i, p2p::CapDesc const& _cap, uint16_t _capID);

	/// Basic destructor.
	virtual ~VaporyPeer();

	/// What is our name?
	static std::string name() { return "vap"; }
	...
	void init(unsigned _hostProtocolVersion, u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash, std::shared_ptr<VaporyHostDataFace> _hostData, std::shared_ptr<VaporyPeerObserverFace> _observer);
...
private:
	...
	std::shared_ptr<VaporyPeerObserverFace> m_observer;
	std::shared_ptr<VaporyHostDataFace> m_hostData;
};
