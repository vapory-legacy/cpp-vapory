...
const char* VMTraceChannel::name() { return "VVM"; }
...
{
	m_t = _transaction;
	m_baseGasRequired = m_t.baseGasRequired(m_sealEngine.vvmSchedule(m_envInfo.number()));
	try
	{
...
bool Executive::call(CallParameters const& _p, u256 const& _gasPrice, Address const& _origin)
{
	...
	if (m_sealEngine.isPrecompiled(_p.codeAddress, m_envInfo.number()))
	{
		...
		if (_p.gas < g)
		{
			...
			// because the bug in both Gvap and Parity led to deleting RIPEMD precompiled in this case
			// see https://github.com/vaporyco/go-vapory/pull/3341/files#diff-2433aa143ee4772026454b8abd76b9dd
...
bool Executive::executeCreate(Address const& _sender, u256 const& _endowment, u256 const& _gasPrice, u256 const& _gas, bytesConstRef _init, Address const& _origin)
{
	...
	// Transfer vapor before deploying the code. This will also create new
	...
	if (m_ext)
	{
		...
		try
		{
			...
			if (m_isCreation)
			{
				...
				if (out.size() > m_ext->vvmSchedule().maxCodeSize)
					BOOST_THROW_EXCEPTION(OutOfGas());
				else if (out.size() * m_ext->vvmSchedule().createDataGas <= m_gas)
				{
					if (m_res)
						m_res->codeDeposit = CodeDeposit::Success;
					m_gas -= out.size() * m_ext->vvmSchedule().createDataGas;
				}
				else
				{
					if (m_ext->vvmSchedule().exceptionalFailedCodeDeposit)
						BOOST_THROW_EXCEPTION(OutOfGas());
					else
					{
						if (m_res)
...
bool Executive::finalize()
{
	...
	if (m_ext)
		m_ext->sub.refunds += m_ext->vvmSchedule().suicideRefundGas * m_ext->sub.suicides.size();
