...
#include <libvapory/LastBlockHashesFace.h>
...
{
	cout
		<< "Usage vapvm <options> [trace|stats|output|test] (<file>|-)\n"
		<< "Transaction options:\n"
...
void version()
{
	cout << "vapvm version " << dev::Version << "\n";
...
class LastBlockHashes: public vap::LastBlockHashesFace
{
public:
...
int main(int argc, char** argv)
{
	...
	if (mode == Mode::Statistics)
	{
		cout << "Gas used: " << res.gasUsed << " (+" << t.baseGasRequired(se->vvmSchedule(envInfo.number())) << " for transaction, -" << res.gasRefunded << " refunded)\n";
