macro(configure_project)
	...
	set(TESTVAP_ARGS "" CACHE STRING "Testvap arguments for ctest tests")
	...
	option(VVMJIT "Build with VVMJIT module enabled" OFF)
	...
	if (FATDB)
		add_definitions(-DVAP_FATDB)
	endif ()

	# ROCKSDB is an option to build Vapory against Facebook's RocksDB instead
	# of LevelDB
	if (ROCKSDB)
		add_definitions(-DVAP_ROCKSDB)
	endif ()

	if (PARANOID)
		add_definitions(-DVAP_PARANOIA)
	endif ()

	if (VMTRACE)
		add_definitions(-DVAP_VMTRACE)
	endif ()
...
macro(print_config)
	...
	message("-- VVMJIT           Build LLVM-based JIT VVM                 ${VVMJIT}")
	message("-- HERA             Build Hera eWASM VM                      ${HERA}")
	message("------------------------------------------------------------- tests")
	message("-- FASTCTEST        Run only test suites in ctest            ${FASTCTEST}")
	message("-- TESTVAP_ARGS     Testvap arguments in ctest:               ")
	message("                    ${TESTVAP_ARGS}")
	message("------------------------------------------------------------------------")
	message("")
endmacro()
