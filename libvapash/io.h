...
// Reference: https://github.com/vaporyco/wiki/wiki/Vapash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see vapash_io_prepare
enum vapash_io_rc {
	VAPASH_IO_FAIL = 0,           ///< There has been an IO failure
	VAPASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	VAPASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	VAPASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of vapash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * vapash_full_t
 */
#ifdef VAPASH_PRINT_CRITICAL_OUTPUT
#define VAPASH_CRITICAL(...)							\
	do													\
	{													\
		printf("VAPASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for vapash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the vapash
 ...
 *                           https://github.com/vaporyco/wiki/wiki/Vapash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum vapash_io_rc
 */
 ...
 */
FILE* vapash_fopen(char const* file_name, char const* mode);
 ...
 */
int vapash_fseek(FILE* f, size_t offset, int origin);
 ...
 *                         error returns NULL
 */
char* vapash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool vapash_mkdir(char const* dirname);
 ...
 */
bool vapash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* vapash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/vaporyco/wiki/wiki/Vapash-DAG
 *
 ...
 */
bool vapash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool vapash_io_mutable_name(
	uint32_t revision,
