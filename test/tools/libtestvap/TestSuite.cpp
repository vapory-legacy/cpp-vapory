...
#include <test/tools/libtestvap/Stats.h>
...
void TestSuite::executeTest(string const& _testFolder, fs::path const& _jsonFileName) const
{
	...
	BOOST_REQUIRE_MESSAGE(s.length() > 0, "Contents of " << boostTestPath.string() << " is empty. Have you cloned the 'tests' repo branch develop and set VAPORY_TEST_PATH to its path?");
	json_spirit::read_string(s, v);
