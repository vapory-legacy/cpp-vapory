...
BOOST_AUTO_TEST_CASE(keypairs)
{
	...
	vap::Transaction t(1000, 0, 0, h160(fromHex("944400f4b88ac9589a0f17ed4671da26bddb668b")), {}, 0, p.secret());
	auto rlp = t.rlp(vap::WithoutSignature);
	auto expectedRlp = "dc80808094944400f4b88ac9589a0f17ed4671da26bddb668b8203e880";
	BOOST_CHECK_EQUAL(toHex(rlp), expectedRlp);
	rlp = t.rlp(vap::WithSignature);
