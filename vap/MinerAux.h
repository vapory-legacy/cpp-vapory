...
//
// Bob has already done just that in https://github.com/bobsummerwill/cpp-vapory/commits/cmake_fixes/vapminer,
// and we should apply those changes back to 'develop'.  It is probably best to defer that cleanup
// until after attempting to backport the Genoil vapminer changes, because they are fairly extensive
...
//
// See https://github.com/bobsummerwill/cpp-vapory/commit/53af845268b91bc6aa1dab53a6eac675157a072b
// See https://github.com/bobsummerwill/cpp-vapory/commit/3b9e581d7c04c637ebda18d3d86b5a24d29226f4
//
// More generally, the fact that nearly all of the code for vapminer is actually in the 'MinerAux.h'
...
inline std::string credits()
{
	...
	out
		<< "cpp-vapory " << dev::Version << endl
		<< "  By cpp-vapory contributors, (c) 2013-2016." << endl
...
{
	static const char* name() { return VapGreen "miner"; }
...
public:
		...
		else if ((arg == "-w" || arg == "--check-pow") && i + 4 < argc)
		{
			...
			try
			{
				...
				if (m.size() == 64 || m.size() == 66)
					...
					seedHash = VapashAux::seedHash(stol(m));
				m = boost::to_lower_copy(string(argv[++i]));
				bi.setDifficulty(u256(m));
				auto boundary = Vapash::boundary(bi);
				m = boost::to_lower_copy(string(argv[++i]));
				Vapash::setNonce(bi, h64(m));
				auto r = VapashAux::eval(seedHash, powHash, h64(m));
				bool valid = r.value < boundary;
				cout << (valid ? "VALID :-)" : "INVALID :-(") << endl;
				cout << r.value << (valid ? " < " : " >= ") << boundary << endl;
				cout << "  where " << boundary << " = 2^256 / " << bi.difficulty() << endl;
				cout << "  and " << r.value << " = vapash(" << powHash << ", " << h64(m) << ")" << endl;
				cout << "  with seed as " << seedHash << endl;
				if (valid)
					cout << "(mixHash = " << r.mixHash << ")" << endl;
				cout << "SHA3( light(seed) ) = " << sha3(VapashAux::light(Vapash::seedHash(bi))->data()) << endl;
				exit(0);
			}
			catch (...)
			{
	...
	{
		if (m_minerType == "cpu")
			VapashCPUMiner::setNumInstances(m_miningThreads);
...
private:
	...
	void doBenchmark(std::string _m, unsigned _warmupDuration = 15, unsigned _trialDuration = 3, unsigned _trials = 5)
	{
		...
		cdebug << Vapash::boundary(genesis);

		GenericFarm<VapashProofOfWork> f;
		...
		f.onSolutionFound([&](VapashProofOfWork::Solution) { return false; });

		string platformInfo = VapashCPUMiner::platformInfo();
		cout << "Benchmarking on platform: " << platformInfo << endl;

		cout << "Preparing DAG..." << endl;
		Vapash::ensurePrecomputed(0);
