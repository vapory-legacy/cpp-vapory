...
or the second is to modify the environment to somvaping sensible (least
...
//  double blockFees = 15.0;
    ...
    clientNetworking.add_options()
        ("bootstrap,b",  "Connect to the default Vapory peer servers (default unless --no-discovery used).")
    ...
    if (mode == OperationMode::ImportSnapshot)
    {
        try
        {
            ...
            importer.import(*snapshotStorage, web3.vapory()->blockChain().genesisHash());
...
//  std::shared_ptr<vap::BasicGasPricer> gasPricer = make_shared<vap::BasicGasPricer>(u256(double(vapor / 1000) / vaporPrice), u256(blockFees * 1000));
    ...
    if (ipc)
    {
        using FullServer = ModularServer<
            ...
            rpc::AdminVapFace, rpc::AdminNetFace,
