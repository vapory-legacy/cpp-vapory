...
#include <libweb3jsonrpc/AdminVap.h>
...
void help()
{
	...
	cout
		...
		<< "    --bootstrap  Connect to the default Vapory peer servers (default unless --no-discovery used).\n"
		<< "    --no-bootstrap  Do not connect to the default Vapory peer servers (default only when --no-discovery is used).\n"
...
string vapCredits(bool _interactive = false)
{
...
void version()
{
	cout << "vap version " << dev::Version << "\n";
	cout << "vap network protocol version: " << dev::vap::c_protocolVersion << "\n";
	cout << "Client database version: " << dev::vap::c_databaseVersion << "\n";
	cout << "Build: " << DEV_QUOTED(VAP_BUILD_PLATFORM) << "/" << DEV_QUOTED(VAP_BUILD_TYPE) << "\n";
	exit(0);
}
...
string pretty(h160 _a, dev::vap::State const& _st)
{
	string ns;
	h256 n;
...
void stopSealingAfterXBlocks(vap::Client* _c, unsigned _start, unsigned& io_mining)
{
	try
	{
		if (io_mining != ~(unsigned)0 && io_mining && asVapashClient(_c)->isMining() && _c->blockChain().details().number - _start == io_mining)
		{
			_c->stopSealing();
...
//	double blockFees = 15.0;
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		{
			chainParams = ChainParams(genesisInfo(vap::Network::MainNetwork), genesisStateRoot(vap::Network::MainNetwork));
			chainConfigIsSet = true;
		}
		else if (arg == "--ropsten" || arg == "--testnet")
		{
			chainParams = ChainParams(genesisInfo(vap::Network::Ropsten), genesisStateRoot(vap::Network::Ropsten));
			chainConfigIsSet = true;
		}
	...
	if (!configJSON.empty())
	{
		...
		catch (...)
		{
			...
			cerr << "sample: \n" << genesisInfo(vap::Network::MainNetworkTest) << "\n";
			return 0;
		}
	}
...
//		c_gasFloorTarget = gasFloor;
	...
	if (g_logVerbosity > 0)
		cout << VapGrayBold "cpp-vapory, a C++ Vapory client" VapReset << "\n";
	...
	auto caps = useWhisper ? set<string>{"vap", "shh"} : set<string>{"vap"};
	...
	if (!extraData.empty())
		web3.vapory()->setExtraData(extraData);

	auto toNumber = [&](string const& s) -> unsigned {
		if (s == "latest")
			return web3.vapory()->number();
		if (s.size() == 64 || (s.size() == 66 && s.substr(0, 2) == "0x"))
			return web3.vapory()->blockChain().number(h256(s));
		try {
			return stol(s);
		}
		catch (...)
	...
	if (mode == OperationMode::Export)
	{
		...
		{
			bytes block = web3.vapory()->blockChain().block(web3.vapory()->blockChain().numberHash(i));
			switch (exportFormat)
			{
	...
	if (mode == OperationMode::Import)
	{
		...
		while (in.peek() != -1)
		{
			...
			switch (web3.vapory()->queueBlock(block, safeImport))
			{
			...
			tuple<ImportRoute, bool, unsigned> r = web3.vapory()->syncQueue(10);
			imported += get<2>(r);
			...
			if ((unsigned)e >= last + 10)
			{
				...
				auto d = e - last;
				cout << i << " more imported at " << (round(i * 10 / d) / 10) << " blocks/s. " << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.vapory()->number() << ")" << "\n";
				last = (unsigned)e;
		...
		while (moreToImport)
		{
			...
			tie(ignore, moreToImport, ignore) = web3.vapory()->syncQueue(100000);
		}
		double e = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - t).count() / 1000.0;
		cout << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.vapory()->number() << ")\n";
		return 0;
	}
			...
			if (!keyManager.load(masterPassword) && masterSet)
			{
				while (true)
				{
					...
					if (keyManager.load(masterPassword))
						break;
					cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " << (getDataDir("vapory") / fs::path("keys.info")).string() << "\n";
				}
			}
		}
		else
	...
	if (mode == OperationMode::ImportSnapshot)
	{
		try
		{
			auto stateImporter = web3.vapory()->createStateImporter();
			auto blockChainImporter = web3.vapory()->createBlockChainImporter();
...
//	std::shared_ptr<vap::BasicGasPricer> gasPricer = make_shared<vap::BasicGasPricer>(u256(double(vapor / 1000) / vaporPrice), u256(blockFees * 1000));
	std::shared_ptr<vap::TrivialGasPricer> gasPricer = make_shared<vap::TrivialGasPricer>(askPrice, bidPrice);
	vap::Client* c = nodeMode == NodeMode::Full ? web3.vapory() : nullptr;
	if (c)
	{
		c->setGasPricer(gasPricer);
		DEV_IGNORE_EXCEPTIONS(asVapashClient(c)->setShouldPrecomputeDAG(m.shouldPrecompute()));
	...
	if (testingMode)
		...
		authenticator = [&](TransactionSkeleton const& _t, bool isProxy) -> bool {
			...
			string r = getResponse(_t.userReadable(isProxy,
				...
				{
					h256 contractCodeHash = web3.vapory()->postState().codeHash(_t.to);
	...
	if (jsonRPCURL > -1 || ipc)
	{
		using FullServer = ModularServer<
			rpc::VapFace, rpc::WhisperFace,
			rpc::NetFace, rpc::Web3Face, rpc::PersonalFace,
			rpc::AdminVapFace, rpc::AdminNetFace, rpc::AdminUtilsFace,
			rpc::DebugFace, rpc::TestFace
		>;

		sessionManager.reset(new rpc::SessionManager());
		accountHolder.reset(new SimpleAccountHolder([&](){ return web3.vapory(); }, getAccountPassword, keyManager, authenticator));
		auto vapFace = new rpc::Vap(*web3.vapory(), *accountHolder.get());
		rpc::TestFace* testVap = nullptr;
		if (testingMode)
			testVap = new rpc::Test(*web3.vapory());

		if (jsonRPCURL >= 0)
		{
			rpc::AdminVap* adminVap = nullptr;
			rpc::PersonalFace* personal = nullptr;
			rpc::AdminNet* adminNet = nullptr;
			rpc::AdminUtils* adminUtils = nullptr;
			if (adminViaHttp)
			{
				personal = new rpc::Personal(keyManager, *accountHolder, *web3.vapory());
				adminVap = new rpc::AdminVap(*web3.vapory(), *gasPricer.get(), keyManager, *sessionManager.get());
			...
			jsonrpcHttpServer.reset(new FullServer(
				vapFace, new rpc::Whisper(web3, {}),
				new rpc::Net(web3), new rpc::Web3(web3.clientVersion()), personal,
				adminVap, adminNet, adminUtils,
				new rpc::Debug(*web3.vapory()),
				testVap
			));
		...
		if (ipc)
		{
			jsonrpcIpcServer.reset(new FullServer(
				vapFace, new rpc::Whisper(web3, {}), new rpc::Net(web3),
				new rpc::Web3(web3.clientVersion()), new rpc::Personal(keyManager, *accountHolder, *web3.vapory()),
				new rpc::AdminVap(*web3.vapory(), *gasPricer.get(), keyManager, *sessionManager.get()),
				new rpc::AdminNet(web3, *sessionManager.get()),
				new rpc::AdminUtils(*sessionManager.get()),
				new rpc::Debug(*web3.vapory()),
				testVap
			));
			auto ipcConnector = new IpcServer("gvap");
