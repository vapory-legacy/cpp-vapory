...
// EIP_616                - SIMD
//
// VVM_SWITCH_DISPATCH    - dispatch via loop and switch
// VVM_JUMP_DISPATCH      - dispatch via a jump table - available only on GCC
//
// VVM_USE_CONSTANT_POOL  - constants unpacked and ready to assign to stack
//
// VVM_REPLACE_CONST_JUMP - pre-verified jumps to save runtime lookup
//
// VVM_TRACE              - provides various levels of tracing
...
#ifndef VVM_JUMP_DISPATCH
	#ifdef __GNUC__
		#define VVM_JUMP_DISPATCH true
	#else
		#define VVM_JUMP_DISPATCH false
	#endif
#endif
#if VVM_JUMP_DISPATCH
	#ifndef __GNUC__
		#error "address of label extension available only on Gnu"
	#endif
#else
	#define VVM_SWITCH_DISPATCH true
#endif

#ifndef VVM_OPTIMIZE
	#define VVM_OPTIMIZE true
#endif
#if VVM_OPTIMIZE
	#define VVM_REPLACE_CONST_JUMP true
	#define VVM_USE_CONSTANT_POOL true
	#define VVM_DO_FIRST_PASS_OPTIMIZATION ( \
				VVM_REPLACE_CONST_JUMP || \
				VVM_USE_CONSTANT_POOL \
			)
#endif
...
///////////////////////////////////////////////////////////////////////////////
//
// set VVM_TRACE to 3, 2, 1, or 0 for lots to no tracing to cerr
//
#ifndef VVM_TRACE
	#define VVM_TRACE 0
#endif
...
// Executive swallows exceptions in some circumstances
	...
	#define THROW_EXCEPTION(X) \
		((cerr << "!!! VVM EXCEPTION " << (X).what() << endl), abort())
#else
	#if VVM_TRACE > 0
		#define THROW_EXCEPTION(X) \
			((cerr << "!!! VVM EXCEPTION " << (X).what() << endl), BOOST_THROW_EXCEPTION(X))
	#else
