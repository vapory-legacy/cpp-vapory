...
public:
	EnvInfo(BlockHeader const& _current, LastBlockHashesFace const& _lh, u256 const& _gasUsed):
		...
		m_gasUsed(_gasUsed)
	{}
	// Constructor with custom gasLimit - used in some synthetic scenarios like vap_estimateGas	RPC method
 ...
 */
class ExtVMFace: public vvm_context
{
public:
	/// Null constructor.
	ExtVMFace() = default;
	...
	/// Return the VVM gas-price schedule for this execution context.
	virtual VVMSchedule const& vvmSchedule() const { return DefaultSchedule; }
...
inline vvm_uint160be toVvmC(Address const& _addr)
{
	return reinterpret_cast<vvm_uint160be const&>(_addr);
}

inline vvm_uint256be toVvmC(h256 const& _h)
{
	return reinterpret_cast<vvm_uint256be const&>(_h);
}
