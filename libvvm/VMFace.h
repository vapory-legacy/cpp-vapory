...
#define VAP_SIMPLE_EXCEPTION_VM(X) struct X: VMException { const char* what() const noexcept override { return #X; } }
VAP_SIMPLE_EXCEPTION_VM(BadInstruction);
VAP_SIMPLE_EXCEPTION_VM(BadJumpDestination);
VAP_SIMPLE_EXCEPTION_VM(OutOfGas);
VAP_SIMPLE_EXCEPTION_VM(OutOfStack);
VAP_SIMPLE_EXCEPTION_VM(StackUnderflow);
VAP_SIMPLE_EXCEPTION_VM(DisallowedStateChange);
VAP_SIMPLE_EXCEPTION_VM(BufferOverrun);
