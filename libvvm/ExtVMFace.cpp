...
static_assert(sizeof(Address) == sizeof(vvm_uint160be), "Address types size mismatch");
static_assert(alignof(Address) == alignof(vvm_uint160be), "Address types alignment mismatch");

inline Address fromVvmC(vvm_uint160be const& _addr)
{
...
int accountExists(vvm_context* _context, vvm_uint160be const* _addr) noexcept
{
...
size_t getCode(byte const** o_code, vvm_context* _context, vvm_uint160be const* _addr)
{
	auto& env = static_cast<ExtVMFace&>(*_context);
	Address addr = fromVvmC(*_addr);
	if (o_code != nullptr)
...
void selfdestruct(
	vvm_context* _context,
	vvm_uint160be const* _addr,
	vvm_uint160be const* _beneficiary
) noexcept
{
	(void) _addr;
	auto& env = static_cast<ExtVMFace&>(*_context);
...
void call(vvm_result* o_result, vvm_context* _context, vvm_message const* _msg) noexcept
{
	...
	// for VVM in general.
	o_result->code = success ? VVM_SUCCESS : VVM_REVERT;
